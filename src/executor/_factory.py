...
_BACKENDS = {
    ...
    "qiskit": {
        "module": "executor.qiskit.qiskit_executor",
...
def create_executor(backend: str, **kwargs):
    ...
    Returns:
        An instance of the corresponding ``ExecutorBase`` subclass.

    Raises:
        ValueError: If *backend* is not one of the supported backends.
        ImportError: If the third-party library required by the backend is not installed.
    """
    key = backend.lower()
    ...
    try:
        ...
        raise ImportError(
            f"Backend {backend!r} requires an extra dependency. "
