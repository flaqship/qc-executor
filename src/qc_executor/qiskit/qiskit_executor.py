...
from qc_executor.qiskit._sympy_bridge import to_qiskit_params
...
def _resolve_derivative_name(name):
    """Translate a derivative request into what the OpTree layer differentiates.

    A framework-independent :class:`~qc_executor.parameters.Parameter` becomes
    the matching Qiskit ``ParameterVectorElement`` (from the shared factory, so
    it is the very object inside the compiled circuit), a
    :class:`~qc_executor.parameters.Parameters` vector its name, and a tuple is
    resolved element-wise.  Strings and Qiskit objects pass through.

    Args:
        name: One entry of the ``derivative`` arguments.

    Returns:
        The resolved entry.
    """
    if isinstance(name, tuple):
        return tuple(_resolve_derivative_name(entry) for entry in name)
    if isinstance(name, Parameters):
        return name.name
    if isinstance(name, Parameter):
        return to_qiskit_params([name])[0]
    return name
...
class QiskitExecutor(ExecutorBase):
    ...
    * ``"aer"`` — the general-purpose ``AerSimulator``: real shot-based
      sampling instead of ``"statevector"``'s analytic noise model, so the two
      are not necessarily numerically equivalent even though both target the
      ...
      pin one.
    * ``"aer_statevector"`` — shorthand for exactly that:
      ``AerSimulator(method="statevector")`` with real shot-based sampling.
    ...
    * A pre-configured Qiskit primitive (``BaseSamplerV2`` / ``BaseEstimatorV2``
    ...
    Args:
        backend: Backend to use for execution.  Accepts:
            ``"statevector"`` (default), ``"aer"`` or ``"aer_statevector"`` string shortcuts, a Qiskit
    ...
    def __init__(
        ...
        # ── 3. Local simulator backends (string shortcuts) ────────────────
            ...
            if backend == "statevector":
                ...
                # "aer" with the simulation method pinned to the statevector.
                aer_simulator_cls = _load_aer_simulator()
    ...
    def _prepare_parameter_dicts(
        ...
        **parameters,
        ...
        def _param_dimension(p) -> int:
            # The vector's own length is no guide: the shared parameter
            # factory grows one vector per name for the whole process, so
            # only the highest element actually referenced counts.
            return p.index + 1 if hasattr(p, "index") else 1
        ...
        # (highest referenced element + 1, or 1 for a standalone Parameter).
        named_params: dict = {}
        for qobj in (*circuits, *observables):
            for p in qobj.parameters:
                name = _param_name(p)
                if name in parameters:
                    named_params[name] = max(named_params.get(name, 0), _param_dimension(p))
    ...
    def _expectation_value_derivatives(
        ...
        **parameter_values,
        ...
        # Framework-independent parameters are resolved to their Qiskit
        # counterparts, which is what the OpTree machinery differentiates.
        derivative_params = tuple(_resolve_derivative_name(dp) for dp in derivative_params)
        ...
        expected_rank = int(is_batched) + int(isinstance(observable, list))

        def _squeeze_circuit_axis(value) -> np.ndarray:
            """Drop the single-circuit axes OpTree keeps in derivative results.

            A derivative evaluates as ``([batch], n_circuits, [n_observables])``
            -- with one more nesting level per differentiation for the
            higher-order case -- and only one circuit ever reaches this method
            (the base class expands circuit lists), so those axes are always
            of length one.  Removing them here is what makes the layout below,
            ``([batch], [n_observables], n_parameters)``, line up with the
            other backends.
            """
            value = np.asarray(value)
            axis = 1 if is_batched else 0
            while value.ndim > expected_rank and value.ndim > axis and value.shape[axis] == 1:
                value = np.take(value, 0, axis=axis)
            return value
        ...
        def _derivative_for_single_param(p) -> float:
            ...
            return _squeeze_circuit_axis(total)
        ...
        def _derivative_for_param_tuple(param_tuple: tuple) -> float:
            ...
            return _squeeze_circuit_axis(
                OpTreeEvaluate.evaluate_with_estimator(
                    circuit=circ_tree,
                    operator=obs_tree,
                    dictionary_circuit=circuit_dict,
                    dictionary_operator=observable_dict,
        ...
        for dp in derivative_params:
            if isinstance(dp, str):
                ...
                if len(matching) == 1:
                    ...
                    # Parameters go last: ([batch], [n_observables], n_parameters).
                    stacked = np.array([_derivative_for_single_param(p) for p in matching])
                    results[dp] = np.moveaxis(stacked, 0, -1)
                ...
                # One trailing axis per tuple position, after the batch and
                # observable axes: ([batch], [n_observables], *shape).
                value = flat.reshape(shape + flat.shape[1:])
                results[dp] = np.moveaxis(
                    value, tuple(range(len(shape))), tuple(range(-len(shape), 0))
                )
            else:
