...
def test_native_pauli_label_convention(executor):
    ...
    z_on_qubit_1 = QiskitOperator.from_quantum_operator(SparsePauliOp(["ZI"], [1.0]))
    z_on_qubit_0 = QiskitOperator.from_quantum_operator(SparsePauliOp(["IZ"], [1.0]))
...
def test_expectation_value_matches_reference(executor):
    ...
    result = executor.expectation_value(
        circuit, QiskitOperator.from_quantum_operator(operator), theta=values
    )
...
def test_derivatives_return_format(executor):
    ...
    circuit = _ansatz()
    operator = QiskitOperator.from_quantum_operator(SparsePauliOp(["ZZ"], [1.0]))
    values = [0.3, 0.8]
...
def test_derivatives_match_finite_differences(executor):
    ...
    gradient = np.asarray(
        executor.expectation_value_derivatives(
            circuit, QiskitOperator.from_quantum_operator(operator), "theta", theta=values
        ),
        dtype=float,
    ).reshape(-1)
...
def test_expectation_value_batch_of_parameter_sets(executor):
    ...
    batched = executor.expectation_value(
        circuit, QiskitOperator.from_quantum_operator(operator), theta=value_sets
    )
